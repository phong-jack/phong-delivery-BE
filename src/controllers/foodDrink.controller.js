...
class FoodDrinkControler {
  ...
  async updateFoodDrink(req, res) {
    new SuccessResponse({
      ...
      metadata: await FoodDrinkService.updateFoodDrink({ id: req.params.id, shopId: req.user.shopId, ...req.body })
    }).send(res);
  }
  ...
  async uploadFoodDrinkImage(req, res) {
    ...
    new OK({
      ...
      metadata: await FoodDrinkService.uploadFoodDrinkImage({ id: req.params.id, image: req.file })
    }).send(res);
  }
}
